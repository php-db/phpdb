...
class LaminasBench
{
    ...
    private \Laminas\Db\Adapter\Platform\PlatformInterface $platform;
    ...
    public function setUp(): void
    {
        ...
        $this->adapter->query("DELETE FROM actor WHERE actor_id > 200", \Laminas\Db\Adapter\Adapter::QUERY_MODE_EXECUTE);
        $this->adapter->query("ALTER TABLE actor AUTO_INCREMENT = 201", \Laminas\Db\Adapter\Adapter::QUERY_MODE_EXECUTE);
    }
    ...
    {
        $select = new \Laminas\Db\Sql\Select('film');
        $select->columns(['film_id', 'title', 'actor_count' => new \Laminas\Db\Sql\Expression('COUNT(film_actor.actor_id)')]);
        $select->join('film_actor', 'film.film_id = film_actor.film_id', []);
    ...
    public function bench6a_Subquery(): void
    {
        ...
        $subSelect->columns(['rental_count' => new \Laminas\Db\Sql\Expression('COUNT(*)')]);
        $subSelect->where(new \Laminas\Db\Sql\Expression('rental.customer_id = customer.customer_id'));

        $select = new \Laminas\Db\Sql\Select('customer');
    ...
    {
        $select = new \Laminas\Db\Sql\Select('film');
        $select->columns(['rating', 'count' => new \Laminas\Db\Sql\Expression('COUNT(*)'), 'avg_length' => new \Laminas\Db\Sql\Expression('AVG(length)')]);
        $select->group('rating');
        $stmt = $this->sql->prepareStatementForSqlObject($select);
        $result = $stmt->execute();
        foreach ($result as $row) {}
